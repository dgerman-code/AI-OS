...
|---|---|---|---|
| `role.product_manager_business_analyst` | `LEAD_ROLE` (S1), `CONTRIBUTING_ROLE` (S5, S6) | S1, S5, S6 | Owns requirements and acceptance criteria. Does not own architecture, security or release. Does not approve its own scope — `decision.product_scope_approval` is human. |
| `role.ux_ui_information_architecture_specialist` | `CONTRIBUTING_ROLE` | S1, S2 | Owns interaction and interface design and information architecture. Triggered where the change is user-facing. |
| `role.solution_architect` | `LEAD_ROLE` (S2), `CONSULTED_ROLE` (S3, on deviation), `CONTRIBUTING_ROLE` (S6) | S2, S3, S6 | Owns the solution architecture specification and architecture decision records. Does not own the security conclusion or the release. |
| `role.data_database_architect` | `CONTRIBUTING_ROLE` | S2, S3 | Owns the data architecture specification and migration design. Triggered where the data model changes. |
| `role.full_stack_software_engineer` | `CONTRIBUTING_ROLE` | S3, S5 | Owns the source change within the approved architecture. Does not alter the architecture position unilaterally. |
| `role.integration_api_engineer` | `CONTRIBUTING_ROLE` | S3, S5 | Owns integration and API contract implementation. Triggered by interface change. |
| `role.database_data_engineer` | `CONTRIBUTING_ROLE` | S3, S5 | Owns migration scripts and data pipeline implementation. Does not execute production migration. |
| `role.platform_devops_engineer` | `CONTRIBUTING_ROLE` | S3, S6 | Owns the deployment pipeline and environment configuration. **Building a pipeline is not authority to release through it.** |
| `role.security_engineer` | `CONTRIBUTING_ROLE` | S2, S4, S6 | Owns threat modelling, security control design and the security control assessment. This conclusion is not obtainable through the architecture Roles. |
| `role.software_qa_test_automation_specialist` | `LEAD_ROLE` (S5), `CONTRIBUTING_ROLE` (S6) | S5, S6 | Owns test strategy, automation and test evidence, and defect records. Does not approve release and does not defer defects. |
...
- **Participating Roles:** `role.solution_architect` (`LEAD_ROLE`), `role.data_database_architect` (`CONTRIBUTING_ROLE`, where data changes), `role.security_engineer` (`CONTRIBUTING_ROLE`), `role.ux_ui_information_architecture_specialist` (`CONTRIBUTING_ROLE`, where user-facing).
...
- **Participating Roles:** `role.full_stack_software_engineer`, `role.integration_api_engineer`, `role.database_data_engineer`, `role.platform_devops_engineer` (all `CONTRIBUTING_ROLE`, each where its layer is in scope); `role.solution_architect` (`CONSULTED_ROLE`, on deviation from the S2 design position).
- **Activities:** source implementation; API and integration contract implementation; migration script development; security control implementation; pipeline and environment configuration.
- **Artifact Contributions:** `artifact.source_code_change` (owned by `role.full_stack_software_engineer`); `artifact.integration_contract_specification` (owned by `role.integration_api_engineer`); `artifact.database_migration_script` (owned by `role.database_data_engineer`); `artifact.deployment_pipeline` (owned by `role.platform_devops_engineer`); `artifact.security_control_implementation` (owned by `role.security_engineer`).
