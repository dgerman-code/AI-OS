...
**Current result: `=== 160/160 PASS ===`.** Normal, `--verbose` and `--json` modes all report the same total. The harness asserts that this document states the total and the group counts the suite actually emits, so a stale number here is a failure rather than a cosmetic slip.

**Scope boundary.** These 160 are **offline, deterministic checks over committed content**. Remote repository state and the configuration of any engine, queue, database or forge are **not provable offline and are not claimed**.

| Group | Checks | Covers |
|---|---:|---|
| `identity` | 12 | The 21-object chain verbatim; each named collapse denied **with its consequence**; the eight load-bearing denials individually; Role activated not instantiated; **no seniority variants**; runtime ID ≠ governance identity; router and orchestrator remain two components; **no normative artifact asserts a denied collapse**, read as fail-closed guarded-pair co-occurrence — an aside is removed, the link is cut at each coordination boundary and only the predicate **attached to the pair** is read, so an unrelated earlier negation or property cannot license a later positive equation, and an unproven relation is an offence; **the 212 guarded pairs are the full closure of every denial chain**, proved twice by two independent parsing paths; and a negative control requiring both closure checks to reject four weakened pair sets; the contradiction grammar driven by its own collapse and denial probes |
...
| `scope` | 9 | Every boundary named; one scope per run, bound at intake; crossing needs an approved mechanism; a sub-run narrows and never widens; scope mismatch stops the run; **a controlled normative grammar that parses every scope-crossing span of the RENDERED text** — the same semantic reader the identity invariants use, so emphasis, code spans, comments, entities and links cannot hide the predicate — ALLOW only for a canonical unconditional prohibition or a crossing bound to an approved mechanism in its own construction, REJECT for everything the grammar cannot consume entirely, plus a guard driving the classifier, the rendered-discovery equivalence and the document scan; sensitivity carried never widened; **no scalar sensitivity ceiling** |
