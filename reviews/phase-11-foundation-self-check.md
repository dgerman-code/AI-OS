...
| Group | Checks | Covers |
|---|---:|---|
| `identity` | 12 | The 21-object chain verbatim; each named collapse denied **with its consequence**; the eight load-bearing denials individually; Role activated not instantiated; **no seniority variants**; runtime ID ≠ governance identity; router and orchestrator remain two components; **no normative artifact asserts a denied collapse**, read as fail-closed guarded-pair co-occurrence — an aside is removed and the predication underneath is read, and an unproven relation is an offence; **the 212 guarded pairs are the full closure of every denial chain**, proved twice by two independent parsing paths; and a negative control requiring both closure checks to reject four weakened pair sets; the contradiction grammar driven by its own collapse and denial probes |
