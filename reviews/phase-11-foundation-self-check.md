...
**Current result: `=== 158/158 PASS ===`.** Normal, `--verbose` and `--json` modes all report the same total. The harness asserts that this document states the total and the group counts the suite actually emits, so a stale number here is a failure rather than a cosmetic slip.

**Scope boundary.** These 158 are **offline, deterministic checks over committed content**. Remote repository state and the configuration of any engine, queue, database or forge are **not provable offline and are not claimed**.

| Group | Checks | Covers |
|---|---:|---|
| `identity` | 10 | The 21-object chain verbatim; each named collapse denied **with its consequence**; the eight load-bearing denials individually; Role activated not instantiated; **no seniority variants**; runtime ID ≠ governance identity; router and orchestrator remain two components; **no normative artifact asserts a denied collapse**, read as guarded-pair co-occurrence rather than by enumerating equivalence verbs; **the 212 guarded pairs are the full closure of every denial chain**, checked against an independent parser and `itertools.combinations`; the contradiction grammar driven by its own collapse and denial probes |
...
| `scope` | 9 | Every boundary named; one scope per run, bound at intake; crossing needs an approved mechanism; a sub-run narrows and never widens; scope mismatch stops the run; **a controlled normative grammar that parses every scope-crossing span** — ALLOW only for a canonical unconditional prohibition or a crossing bound to an approved mechanism in its own construction, and REJECT for everything the grammar cannot consume entirely — plus a thirty-six-span guard driving both the classifier and the document scan; sensitivity carried never widened; **no scalar sensitivity ceiling** |
