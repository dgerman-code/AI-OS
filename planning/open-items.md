...
|---:|---|---|---|
| **PO-1** | **No approved Role owns communication strategy for contested interactions.** A candidate exists on a separate proposal branch and is not approved | Phase 15 may not create a Role (N-1). Until one is approved, a request whose **originally requested** deliverable needs that conclusion is **BLOCKED and escalated** — `F-14 NO_APPROVED_ROLE_OWNS_CONCLUSION` (RS-9 third row, RS-12, RS-13). It is never narrowed into a constrained plan and offered as though it answered the request; a narrower plan exists only for a **new linked `Request`** in which the user changes the deliverable | Role Registry change control |
