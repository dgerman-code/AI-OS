...
| A61 | `Retry`, *reconciliation* | CURRENT | Automatically redispatch an `ATTEMPTED_OUTCOME_UNKNOWN` external effect on the grounds that no effect was observed | Refused in every location that states it: branch R6 blocks and escalates, PO-14 requires a recorded deduplication guarantee or a `CONFIRMED_NOT_APPLIED` reconciliation, and absence of evidence of an effect is never evidence of none |
