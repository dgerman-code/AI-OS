...
| `decision.stage_gate_progression` | The accepted risk is material to the readiness position the named stage gate is decided on | **`SEPARATION_REQUIRED`** | That gate and that risk | Declared from the other end in `decision.stage_gate_progression` and reciprocated here. A gate passed with known risk is a gate passed with known risk. Whoever decided the entity carries it should not also be the one who decides it is no obstacle. |
